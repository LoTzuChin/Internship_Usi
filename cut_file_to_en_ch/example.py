...
# patterm 正規化的部分是單純數字的編號，如：1., 1.1, 1.1.1
pattern = re.compile(r'^(\d\.|\d+(\.\d+)*)\s')
# pattern 正規化部分是小編號部分，相比上方呈現形式較不確定，如：(1), (a), a.
...
def main():
    ...
    with open('output.txt', 'w', encoding='utf-8') as f:
        ...
        # for element in doc.inline_shapes:
        #     f.write(f'[PIC] {element}\n\n\n')
        ...
        for paragraph in doc.paragraphs:
            if not paragraph.text.strip():
                continue
            # 抓取符合 pattern的文字內容
            match = pattern.match(paragraph.text) # <re.Match object; span=(0, 4), match='3.1 '>
            # 抓取符合 pattern_char的文字內容
            match_char = pattern_char.match(paragraph.text) # <re.Match object; span=(0, 4), match='(1) '>

            # 根據抓取的內容判斷要縮進幾排
            # 優先判斷符合 pattren的
            if match:
                numbering = match.group(1)  # 3.1
